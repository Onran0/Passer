...
public class Main {
    ...
    private byte[] generatePassword(int[] type) {
    ...
    private byte[] extractBytesFromInputPassword(char[] passwordInput, int[] type) {
    ...
    private void add() {
        ...
        if(tokensRemain() > 1) {
            ...
            if(type != null) {
                if(tokensRemain() < 1 || !Boolean.parseBoolean(nextToken())) {
    ...
    private void mod() {
        ...
        if("password".equals(propertyName)) {
            boolean[] autogen = { tokensRemain() > 0 && Boolean.parseBoolean(nextToken()) };
            ...
            if(tokensRemain() < 1) {
                ...
                usage("mod");
                return;
            } else argument = nextToken();

            switch(propertyName) {
                case "caption":
                    passwordInfo.setCaption(argument.toCharArray());
                    break;

                case "service":
                    passwordInfo.setService(argument.toCharArray());
                    break;

                case "login":
                    passwordInfo.setLogin(argument.toCharArray());
                    break;
            }

            out.printf("property \"%s\" of pass with id \"%d\" was changed to \"%s\"\n", propertyName, id, argument);
        }
